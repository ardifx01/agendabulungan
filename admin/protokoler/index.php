...
$cari = $_GET['cari'] ?? '';
$query = "SELECT * FROM protokoler WHERE nama LIKE :cari OR unit_kerja LIKE :cari ORDER BY nama ASC";
$stmt = $pdo->prepare($query);
$stmt->execute(['cari' => "%$cari%"]);
$protokoler = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<main class="p-6 ml-64">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold">Data Protokoler</h1>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-200 rounded shadow">
            ...
            <tbody>
                <?php if (count($protokoler) > 0): ?>
                    <?php $no = 1; foreach ($protokoler as $row): ?>
